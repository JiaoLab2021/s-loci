...
class GzChunkReader {
public:
    explicit GzChunkReader(const std::string& file_path, const size_t chunk_size = 1024 * 1024)
        : file_path_(file_path), chunk_size_(chunk_size),
          chunk_buffer_(new char[chunk_size_]), buffer_pos_(0), buffer_end_(0) 
    {
        // The file handle is of type gzFile
    ...
    bool read_line(std::string& line) {
        ...
        while (true) {
            ...
            if (buffer_pos_ == buffer_end_) {
                if (!read_chunk()) {
                    // If the end of the file is reached but there is no newline character, return the last part. Otherwise, read one less line.
                    if (!line.empty()) {
                        return true;
                    }
                    return false;
...
private:
    ...
    size_t buffer_pos_;
    size_t buffer_end_;
    gzFile file_;  // The file handle is of type gzFile
};
